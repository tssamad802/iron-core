...
(function () {
  ...
  /* ── DOM refs ─────────────────────────────────────────── */
  const filterSearch  = document.querySelector('.filter-bar .filter-search input');
  const filterStatus  = document.querySelectorAll('.filter-bar .filter-select')[0];
  const filterTrainer = document.querySelectorAll('.filter-bar .filter-select')[1];
  ...
  function updatePageInfo(visible, total) {
    ...
    if (visible === total) {
      ...
      pageInfo.dataset.original = pageInfo.dataset.original || pageInfo.textContent;
  ...
  function updatePaginationVisibility(filtering) {
    ...
    if (paginationBtns) {
      ...
      paginationBtns.title                  = filtering
        ? 'Clear filters to use pagination'
        : '';
    }
  }
  ...
  (function populateTrainerDropdown() {
    ...
    getRows().forEach(row => {
      ...
      if (txt.includes('n/a') || txt.includes('unassign')) {
        ...
        const raw = (row.querySelector('.trainer-assigned')?.textContent || '').trim();
  ...
  (function persistFiltersInPaginationLinks() {
    function refreshLinks() {
      ...
      document.querySelectorAll('.page-btns a').forEach(a => {
        ...
        if (trainer) url.searchParams.set('trainer', trainer); else url.searchParams.delete('trainer');
        a.href = url.toString();
      });
    }
  ...
  if (topbarSearch) {
    topbarSearch.addEventListener('input', debounce(function () {
